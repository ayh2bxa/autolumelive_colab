...
def training_loop(
    ...
    for module in [G, D, G_ema, T, augment_pipe]:
    ...
    loss = dnnlib.util.construct_class_by_name(device=device, G=G, D=D, T=T, augment_pipe=augment_pipe, **loss_kwargs) # subclass of training.loss.Loss
    phases = []
            ...
            for key, value in snapshot_data.items():
                if isinstance(value, torch.nn.Module):
                    if projected and (key == 'D'):
                        value = value.eval().requires_grad_(False)
                    else:
